...
#tring to get props
=======
>>>>>>> be4df083ba37e248a60ba7b462886695a46b7d88
...
prob_df <- data.frame(x = qnorm(ppoints(20), 
                                    mean = tidied$estimate[4],
                                    sd = tidied$std.error[4])) %>% 
  mutate(crime_rate_diff = ifelse(x > 0, "blue", "orange"))

ggplot(prob_df, aes(x)) +
  geom_dotplot(aes(fill = crime_rate_diff), binwidth = 1300) +
  geom_vline(xintercept = 0, color = "red4", linetype = 1, size = 1.4) +
  scale_fill_manual(
    values = c("orange3","deepskyblue3"),
    guide = "none") +
