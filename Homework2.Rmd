...
#Can't get the density stripes to look right, not sure if it's something wrong with my std. error or what
crime_percent %>%
 ggplot(aes(fct_reorder(offense_category_id, Percent), Percent)) +
  stat_confidence_density(aes(moe = prop_se), fill = "#4375D3", height = 0.6) +
  geom_point(color = "cornflower blue") +
  coord_flip() +
  theme_minimal() +
  #scale_y_continuous(labels = scales::percent, limits = c(NA, .3)) +
...
tab_source_note(summary_df5, source_note = md("Data from [fivethirtyeight](https://fivethirtyeight.com/features/airplane-etiquette-recline-seat/)"))
